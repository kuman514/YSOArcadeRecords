...
import ReviewSearchResultList from '^/src/features/review-article/search-result-list';
...
export async function generateMetadata({
  ...
  return {
    title: `${searchText} 리뷰 검색 결과 :: ${APP_NAME}`,
  };
}

export default async function ReviewSearchPage({ searchParams }: Props) {
  const { searchText } = await searchParams;

  const queryClient = new QueryClient();
  const queryKey = ['search', 'reviews', searchText];
